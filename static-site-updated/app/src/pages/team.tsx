...
// Get unique categories for a given year
const getCategoriesForYear = (year: number | null) => {
  const members = year ? teamData.filter(m => m.teamYear === year) : teamData;
  const categories = Array.from(new Set(members.map(m => (m as any).category || "Team")));
  return categories;
};
        ...
        <div className="container mx-auto px-4 pb-16">
          ...
          >
            <AnimatePresence mode="popLayout">
              {/* Show Micro Intern section header if applicable */}
              {(() => {
                const categories = getCategoriesForYear(selectedYear);
                const hasMultipleCategories = categories.length > 1 || categories.includes("Micro Intern");
                
                if (hasMultipleCategories) {
                  // Group by category
                  return categories.map(category => {
                    const categoryMembers = filteredTeam.filter(m => ((m as any).category || "Team") === category);
